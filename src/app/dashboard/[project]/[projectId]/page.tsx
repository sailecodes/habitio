...
import TodaysHabit from "@/components/dashboard/project/todays-habit";
import { Separator } from "@/components/ui/separator";

export default async function Project({
  params,
}: {
  params: Promise<{ project: string; projectId: string }>;
}) {
  const { project, projectId } = await params;
  const dProject = decodeURIComponent(project);

  return (
    <section className="mx-auto w-full max-w-[1410px] p-10">
      <header className="flex justify-between gap-10">
        {/* Project Name */}
        <span className="text-title">{dProject}</span>
        {/* Streak */}
